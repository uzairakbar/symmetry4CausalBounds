...
objective, the coarse `anchors_c_` for the constraint), same clipping. Gate A75
checks the two against each other in value and against finite differences in
gradient.

CPU only, and single threaded: at m = 32 a GPU launch costs more than the whole
evaluation, and one XLA thread keeps the serial and the loky-chunked solves bit
identical (`BoundedSA._raw_bounds` pins BLAS to one thread for the same reason).
The environment has the CUDA plugin installed, so the platform switch MUST precede
`import jax` in every process that imports this module, workers included.
"""
...
def build_constraint(model, radius):
    ...
    Takes the SAME radius the objective got. The mixin read `self.gamma` here
    instead, which silently evaluates the constraint on a different ball than the
    objective whenever predict(gamma=...) overrides the budget, which is exactly
    what a gamma sweep does.
    """
