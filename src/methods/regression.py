...
class GradientDescentERM(pointEstimator):
    ...
    def _fit(self, X, y, lr=0.01, batch=256, epochs=1, weight_decay=0.0,
             label_smoothing=0.0, optimizer='adam', loss='mse', betas=(0.7, 0.9),
             onecycle=True, init_seed=None, **kwargs):
        import torch
        ...
        self.f.train()
        opt = torch.optim.Adam(self.f.parameters(), lr=lr, weight_decay=weight_decay,
                               betas=tuple(betas))
        ...
        sched = (torch.optim.lr_scheduler.OneCycleLR(
            opt, max_lr=lr, total_steps=int(epochs) * n_batches) if onecycle else None)
        ...
        self.f.eval()
        del Xt, yt
        self._W = np.concatenate([w.detach().cpu().numpy().ravel()
                                  for w in self.f.parameters()])[:, None]
    ...
    def save_state(self, path):
        import torch
        torch.save({'model': self.model, 'input_dim': self._input_dim,
                    'state': {k: v.detach().cpu()
                              for k, v in self.f.state_dict().items()}}, path)
        return path
