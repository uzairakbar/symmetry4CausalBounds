...
from src.experiments.utils.metrics import (trace_S_over_k,        # noqa: E402
                                           rho_hat, evaluate_queries)
...
def main():
    ...
    orchestrator = SimulationOrchestrator(
        seed=42, n_samples=2048, n_experiments=1, sweep_samples=N_STEPS,
        kernel_dim=0, methods=METHODS, hyperparameters={}, n_jobs=8,
        calibrate=False, pad=False, clipy=True,
    )
    ...
    header = (f'{"knob":>9} {"rho":>8} {"trS/k@1.0":>10} {"trS/k@.999":>11} '
              f'{"x=rho*trS":>10} ' + ' '.join(f'{m:>10}' for m in METHODS)
              + f' {"eps_iv":>9}')
    print(header)
    print('-' * len(header))
    ...
    for knob in knobs:
        ...
        widths = {}
        for name in METHODS:
            estimate = models[name].predict(
                data.X_test, **runner.get_predict_kwargs(knob, 0))
            record = evaluate_queries(data.estimand, estimate,
                                      getattr(models[name], 'query_status', None), 0.0)
            widths[name] = record.interval_width

        eps_iv = runner.fit_epsilon_iv(0)
        rows.append((knob, rho, trs_full, trs_trunc, x, widths, eps_iv))
        print(f'{knob:9.4g} {rho:8.4f} {trs_full:10.5f} {trs_trunc:11.5f} '
              f'{x:10.5f} ' + ' '.join(f'{widths[m]:10.5f}' for m in METHODS)
              + f' {eps_iv:9.6f}')
    ...
    print('\n--- monotonicity ---')
    for label, values in (('rho vs knob', rhos), ('tr(S)/k vs knob', trs_full),
                          ('x vs knob', x_values)):
        d = np.diff(values)
        mono = bool((d > 0).all() or (d < 0).all())
        direction = ('increasing' if (d > 0).all() else
                     'decreasing' if (d < 0).all() else 'NON-MONOTONE')
        print(f'  {label:20s} {direction:14s} ' + ('' if mono else
              f'sign flips at knob '
    ...
    print(f'  eps_iv range: {eps_ivs.min():.9f} .. {eps_ivs.max():.9f}  '
          f'(spread {np.ptp(eps_ivs):.3g})')
