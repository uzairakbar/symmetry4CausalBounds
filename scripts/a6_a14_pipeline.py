...
def fixture():
    ...
    nets = {'X': GradientDescentERM().fit(X, y, init_seed=42, epochs=1),
...
def a13_cost_profile(sem, nets, X, GX, y, G, Q):
    ...
    common = dict(n_components=32, calibrate=True, clipy=True,
...
def a7_query_status(sem, nets, X, GX, y, G, Q):
    ...
    check('A7 status_counts sums to n_queries',
          int(record.status_counts.sum()) == len(Q), f'{record.status_counts}')

    # a budget below the floor must gate the WHOLE batch, via _prepare -> None
    starved = IVConstrainedCopSens(gamma=0.1, epsilon_iv=1e-9, outcome_model=nets['GX'],
                                   **common).fit(GX, y, Z=G)
    bounds = starved.predict(Q)
    check('A7 sub-floor budget => all INFEASIBLE',
...
def a8_jax_equals_fd(nets, X, GX, y, G, Q):
    common = dict(n_components=32, calibrate=True, clipy=True, n_jobs=1,
                  mu_clip=DOMNIST_CONFIG.attainable, n_anchors=DOMNIST_CONFIG.n_anchors)
    for name, build in (
        ('PI', lambda jg: CopSensPI(gamma=0.1, outcome_model=nets['X'],
                                    jax_grad=jg, **common).fit(X, y)),
        ('DA+PI_IV', lambda jg: IVConstrainedCopSens(
            gamma=0.1, epsilon_iv=0.12, outcome_model=nets['GX'], jax_grad=jg,
            **common).fit(GX, y, Z=G)),
    ):
...
def a12_training_recipe():
    ...
    check('A12 weight_decay defaults to 0',
          signature.parameters['weight_decay'].default == 0.0)
...
def a9_target_net(sem):
    ...
    """
    sem_test = DoMNISTSEM(seed=42, train=False, target_samples=N_SAMPLES,
                          target_kw=dict(epochs=1), alpha=DOMNIST_CONFIG.alpha,
                          beta=DOMNIST_CONFIG.beta, eta=DOMNIST_CONFIG.eta)
    flat = Flatten()
    ...
    for value, want in ((predicted[last['f'] < 0.5].mean(), 0.2),
                        (predicted[last['f'] > 0.5].mean(), 0.8)):
        check(f'A9 mean at h*={want} within 0.02', abs(float(value) - want) < 0.02,
              f'{float(value):.4f}')
...
def a11_config_strictness():
    good = dict(seed=42, augmentation='translate', gamma=0.1, epsilon=0.05,
                methods=['PI'])
    ...
    try:
        ...
        check('A11 missing outcome_models raises clearly',
              'prefit outcome nets' in str(exc))
...
def a6_perf_fairness():
    ...
    orchestrator = DoMNISTOrchestrator(
        seed=42, n_samples=1024, n_experiments=1, sweep_samples=10,
        methods=['ERM', 'PI'], hyperparameters=dict(epochs=1), n_jobs=16,
        augmentation='translate > rotation', gamma=0.1, epsilon=0.05,
        n_pi=512, n_queries=8, calibrate=True, pad=False, clipy=True,
    )
    nets = {'X': None, 'GX': None}
    from functools import partial
    sweep = orchestrator.build_methods(gamma=.1, epsilon=.05, outcome_models=nets)
    perf = partial(orchestrator.build_methods, n_jobs=1)(
        gamma=.1, epsilon=.05, outcome_models=nets)

    check('A6 sweep models keep the configured n_jobs', sweep['PI']().n_jobs == 16,
          f'{sweep["PI"]().n_jobs}')
    check('A6 perf models are actually serial', perf['PI']().n_jobs == 1,
          f'{perf["PI"]().n_jobs}')

    import inspect
    import src.experiments.base as base
    source = inspect.getsource(base.ExperimentOrchestrator._run_perf)
    check('A6 perf binds n_jobs=1 on the factory too',
...
if __name__ == '__main__':
    ...
    if full:
        N_SAMPLES = 1_200_000
        a9_target_net(DoMNISTSEM(
            seed=42, train=True, target_samples=N_SAMPLES, target_kw=dict(epochs=1),
