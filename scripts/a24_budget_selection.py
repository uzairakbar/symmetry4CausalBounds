...
def a24_bisection():
    ...
    got = min_knob_for_coverage(evaluate, 0.01, 10.0, target=0.95, max_iter=40, tol=1e-4)
    check("A24 bisection finds the true threshold", abs(got["value"] - 0.30) < 3e-4, f"got {got['value']:.6f}")
    check("A24 bisection brackets the threshold", got["bracket"][0] <= 0.30 <= got["bracket"][1] * (1 + 1e-9))
...
def a24_report_schema(path):
    ...
    for leg in ("gamma", "epsilon", "epsilon_iv"):
        ...
        for key in ("value", "achieved_coverage", "bracket", "curve", "target_reachable", "status_counts"):
            check(f"A24 {leg} reports `{key}`", key in entry)

    check(
        "A24 config_patch is exactly the three budgets",
        set(report["config_patch"]) == {"gamma", "epsilon", "epsilon_iv"},
    )
    check(
        "A24 config_patch matches the selected values",
        all(report["config_patch"][k] == report["selected"][k]["value"] for k in ("gamma", "epsilon", "epsilon_iv")),
    )
    ...
    for leg in ("epsilon", "epsilon_iv"):
        ...
        if entry.get("floor"):
            check(
                f"A24 {leg} clears its own floor",
...
if __name__ == "__main__":
    a24_bisection()
    a24_floor_cache()
    a24_report_schema(sys.argv[1] if len(sys.argv) > 1 else "artifacts/domnist-budget_report_smoke.json")
