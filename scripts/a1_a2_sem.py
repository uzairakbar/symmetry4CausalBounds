...
def dump(path):
    ...
    np.savez_compressed(_path(path), X=X, y=y, Xd=Xd, yd=yd, Xp=Xp, y_obs=y_obs, y_do=y_do, ex=ex, digits=digits)
...
def a1(sem):
    ...
    for f in (0.0, 1.0):
        for c in (0.0, 1.0):
            ...
            check(f"obs cell f={f:.0f} C={c:.0f} -> {want}", abs(got - want) < 0.013, f"got {got:.4f}")

    _, y_do = sem.sample(N=N_LAW, intervention=True, seed=3)
    last = sem.last_
    for f in (0.0, 1.0):
        got, want = float(y_do[last["f"] == f].mean()), float(sem.h_star(f))
        check(f"do cell f={f:.0f} -> {want}", abs(got - want) < 0.013, f"got {got:.4f}")
    ...
    check("attainable closed form", np.allclose(sem.attainable, (0.1, 0.9), atol=1e-12), f"{sem.attainable}")

    # h_* is colour-free: the do draw must not depend on C at fixed f
    spread = max(
        abs(float(y_do[(last["f"] == f) & (last["C"] == c)].mean()) - float(sem.h_star(f)))
        for f in (0.0, 1.0)
        for c in (0.0, 1.0)
    )
