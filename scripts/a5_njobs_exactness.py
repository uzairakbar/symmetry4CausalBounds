...
from src.methods.copsens import (CopSensPI, RecentredInvCopSens,  # noqa: E402
...
def main():
    set_seed(42)
    sem = DoMNISTSEM(seed=42, train=True, target_samples=1,
    ...
    common = dict(gamma=0.1, n_components=32, calibrate=True, clipy=True,
                  ...
                  n_anchors=DOMNIST_CONFIG.n_anchors, jax_grad=True)

    cases = {
        'PI': lambda nj: CopSensPI(outcome_model=nets['X'], n_jobs=nj,
                                   **common).fit(X, y),
        'DA+PI': lambda nj: CopSensPI(outcome_model=nets['GX'], n_jobs=nj,
                                      **common).fit(GX, y),
        # eps large enough to clear the floor, so this exercises a FEASIBLE
        # constrained solve rather than the all-INFEASIBLE gate
        'PI_INV': lambda nj: RecentredInvCopSens(
            outcome_model=nets['GX'], epsilon=0.2, n_jobs=nj,
            **common).fit(X, y, GX=GX),
        'DA+PI_IV': lambda nj: IVConstrainedCopSens(
            outcome_model=nets['GX'], epsilon_iv=0.12, n_jobs=nj,
            **common).fit(GX, y, Z=G),
    }
    ...
    for name, build in cases.items():
        ...
        start = time.perf_counter()
        bounds_parallel, status_parallel = (parallel.predict(Q),
                                            parallel.query_status.copy())
        t_parallel = time.perf_counter() - start

        check(f'A5 {name:9s} bounds bit-identical',
              np.array_equal(bounds_serial, bounds_parallel, equal_nan=True),
              f'{t_serial:.1f}s -> {t_parallel:.1f}s ({t_serial / t_parallel:.1f}x)')
        check(f'A5 {name:9s} query_status identical',
              np.array_equal(status_serial, status_parallel))
