...
from src.experiments.utils.constants import (PLOT_CONFIGS,        # noqa: E402
                                             ARTIFACTS_DIRECTORY)
...
def report_real():
    ...
    print('=== resolved on the checked-in sweeps (median over '
          f'{len(SEEDS)} bootstrap seeds) ===')
    header = (f'{"experiment":<16}{"param":<8}{"metric":<14}{"range":>22} '
    ...
    for path in paths:
        ...
        for metric, spec in METRIC_SPECS.items():
            ...
            scale, kwargs = plotting._resolve_scale(
                spec.yscale, plotting._finite(*series), cfg, 'y', limits)
            ...
            print(f'{experiment:<16}{param:<8}{metric:<14}'
...
def synthetic_cases():
    ...
    return {
        ...
        'zero_crossing': {'PI': replicate(np.linspace(-5, 5, steps)),
                          'DA+PI': replicate(np.zeros(steps))},
        'six_decades':   {'PI': replicate(np.logspace(-3, 3, steps)),
                          'DA+PI': replicate(np.full(steps, 1e-3))},
        'single_method': {'PI': replicate(np.linspace(1, 2, steps))},
        'huge_ci':       {'PI': np.column_stack(
                              [np.linspace(1, 2, steps) + 100 * s for s in range(4)])},
        'partial_nan':   {'PI': replicate([np.nan] * 6 + list(np.linspace(1, 5, 6))),
...
def render():
    ...
    print('=== synthetic cases (rendered to '
          f'{ARTIFACTS_DIRECTORY}/{DIAG_EXPERIMENT}/) ===')

    for name, y_results in cases.items():
        before = len(_errors)
        for yscale in ('linear', 'log', 'asinh'):
            plotting.create_sweep_plot(
                x, {k: v.copy() for k, v in y_results.items()},
                xlabel=name, ylabel='y', yscale=yscale,
                experiment=DIAG_EXPERIMENT, fname=f'{name}_{yscale}',
            )
        record = {k: {'a': v, 'b': v} for k, v in y_results.items()}
        plotting.create_scatter_plot(
            record, x, metric_x='a', metric_y='b', xlabel='a', ylabel='b',
            experiment=DIAG_EXPERIMENT, fname=f'scatter_{name}', crosshairs=True,
        )
        print(f'  {name:<16} {"ok" if len(_errors) == before else "ERRORS"}')

    perf = {'PI': {'rates': np.array([0.1, 0.2, 0.6, 0.1]), 'wall_clock': 0.5,
                   'seed_var': 0.02},
            'DA+PI': {'rates': np.array([0.0, 0.0, 0.9, 0.1]), 'wall_clock': 1e-4,
                      'seed_var': 0.5}}
    for overlays in ([], ['wall_clock'], ['wall_clock', 'seed_var']):
        before = len(_errors)
        plotting.create_perf_plot(perf, overlay_metrics=overlays,
                                  experiment=DIAG_EXPERIMENT)
        print(f'  {"perf " + (",".join(overlays) or "no-overlay"):<16} '
              f'{"ok" if len(_errors) == before else "ERRORS"}')
    print()
...
def verdicts():
    ...
    blanked = []
    for name, y_results in cases.items():
        series = [np.nanmean(np.where(np.isinf(v), np.nan, v), axis=1)
                  for v in y_results.values()]
        limits = plotting._limits(series)
        if limits is None:
            continue
    ...
    for name, y_results in cases.items():
        ...
        for requested in ('linear', 'log', 'asinh'):
            scale, _ = plotting._resolve_scale(
                requested, pooled, {}, 'y', plotting._limits(series))
...
def stale_ids():
    ...
    stale = [f'{experiment}.{plot_id}'
             for experiment, plots in PLOT_CONFIGS.items()
             for plot_id in plots if plot_id not in valid]
