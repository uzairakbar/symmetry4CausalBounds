...
| A37 | `a37_major_ticks.py` | at least two labelled major ticks on every axis: the n sweep's ticks on its percentage ladder (6.25 .. 100), the (1, 2, 5) log fallback with plain labels on a pre-ladder n grid (200, 500, 1000), every sweep param on both datasets, the query sweep and the three perf sweep figures, on synthetic inputs; x is the exact grid plus a 2 % margin (no top-tail clip) with the in-grid reference lines drawn inside the frame, r = 1 on the gamma sweep pkls too (`--artifacts DIR`, read only) |
