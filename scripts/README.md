...
| A40 | `a40_eps_star.py` | robustness eps* per dataset: `ROBUSTNESS_EPSILON_TRUE[experiment_name]` reaches the epsilon runner of both orchestrators (sim 3.0 with the tuned DA landing on it, optical 2**-1 with the shipped chain's fixed eps*), the sim constant clears the post-DA radius sqrt(gamma*) and sigma sqrt(gamma*), and on one sim experiment (5 steps) DA+PI coverage dips below 1 at the smallest r, is 1 at r = 1 and never falls under 0.7 |
