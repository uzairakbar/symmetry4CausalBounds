...
| A37 | `a37_major_ticks.py` | at least two labelled major ticks on every axis: the (1, 2, 5) log fallback with plain labels on the n sweep (200, 500, 1000), every sweep param on both datasets, the query sweep and the perf figure, on synthetic inputs; x keeps its exact [min, max] (no top-tail clip on the grid) and the in-grid reference lines draw, r = 1 on the gamma sweep pkls too (`--artifacts DIR`, read only) |
