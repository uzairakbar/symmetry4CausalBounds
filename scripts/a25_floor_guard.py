...
from src.methods.sensitivity_models import (constraint_floor,     # noqa: E402
                                            inv_constraint_terms, iv_constraint_terms)
...
def cvxpy_floor(design, y, gamma, kind, GX=None, Z=None, calibrate=False):
    ...
    A, b = (inv_constraint_terms(design, GX) if kind == 'inv'
            else iv_constraint_terms(design, y, Z))
...
def sim_runner(steps=12):
    ...
    orch = SimulationOrchestrator(
        seed=42, n_samples=2048, n_experiments=1, sweep_samples=steps, kernel_dim=0,
        methods=METHODS, hyperparameters={}, n_jobs=8, calibrate=False, pad=False,
        clipy=True)
    return orch.get_sweep_runner_cls('trS')(
        methods=orch.methods, method_factory=orch.build_methods,
...
def a25_noop_when_feasible():
    ...
    orch = OpticalOrchestrator(
        seed=69, n_samples=200, n_experiments=2, sweep_samples=3, methods=METHODS,
        hyperparameters={}, n_jobs=1, calibrate=False, pad=False, clipy=True,
    ...
    for e in range(2):
        ...
        for kind, attr, fit in (('inv', 'epsilon_star', runner.fit_epsilon),
            ...
            design = data.X if kind == 'inv' else data.GX
            floor = constraint_floor(design, data.y, gamma, kind=kind,
                                     calibrate=runner.calibrate, **kw)
            got = fit(e, 0, data)
            if raw ** 2 >= floor:
                seen_feasible = True
                check(f'A25 no-op, exp {e} {kind} (oracle {raw:.5f}, '
                      f'sqrt(floor) {np.sqrt(floor):.5f})', got == raw,
                      f'got {got:.6f}')
            else:
                check(f'A25 rescue, exp {e} {kind}',
                      abs(got - np.sqrt(FLOOR_GUARD_R * floor)) < 1e-12,
                      f'got {got:.6f}')
...
def a25_rescues_infeasible():
    ...
    for index, knob in enumerate(runner.get_param_range()):
        ...
        floor = constraint_floor(data.GX, data.y, gamma, kind='iv', Z=data.G,
        ...
        if oracle_iv ** 2 < floor:
            ...
            record = evaluate_queries(
                data.estimand,
                models['DA+PI_IV'].predict(data.X_test,
                                           **runner.get_predict_kwargs(knob, 0)),
                getattr(models['DA+PI_IV'], 'query_status', None), 0.0)
            solved = np.isfinite(record.interval_width) and record.interval_width > 0
            rescued += bool(solved)
            check(f'A25 knob {knob:.4g} rescued (budget {oracle_iv:.5f} -> '
                  f'{guarded:.5f})', solved,
                  f'W {record.interval_width:.4f} C {record.coverage:.3f}')
            check(f'A25 knob {knob:.4g} covers at the guarded budget',
                  record.coverage >= 0.95, f'C {record.coverage:.3f}')
        else:
            check(f'A25 knob {knob:.4g} untouched', guarded == oracle_iv,
                  f'{guarded:.6f}')

    check('A25 the sweep had infeasible steps to rescue', nan_steps > 0,
          f'{nan_steps} steps')
    check('A25 every infeasible step was rescued', rescued == nan_steps,
          f'{rescued}/{nan_steps}')
