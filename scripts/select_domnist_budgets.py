...
def probe(model, X_eval, estimand, **predict_kw):
    ...
    record = evaluate_queries(
        estimand, model.predict(X_eval, **predict_kw), getattr(model, "query_status", None), time.time() - started
    )
    return record
...
def min_knob_for_coverage(evaluate, lo, hi, target, max_iter=12, tol=0.01):
    ...
    def at(value):
        ...
        logger.info(f"  probe {value:.6g}: coverage {record.coverage:.4f} width {record.interval_width:.5f}")
        return record

    top = at(hi)
    if top.coverage < target:  # unreachable: report the ceiling honestly
        return dict(value=float(hi), record=top, curve=curve, n_iter=1, bracket=[float(hi), float(hi)], reached=False)
...
def leg_report(selected, floor=None, reference_width=None, ceiling=None, target_reachable=True):
...
def main(args):
    ...
    logger.info(
        f"select: target coverage {target}, n_samples {n_samples:,}, "
    ...
    sem_kw = dict(
        seed=seed,
        net=block.get("net", "domnist-fast"),
        ...
        target_kw=dict(hyperparameters),
    )
    ...
    GX_raw, G = da(X_raw)
    nets = train_pair(
        flat.fit_transform(X_raw), flat.fit_transform(GX_raw), y, init_seed=sem.seed, net=sem.net, **hyperparameters
    )
    ...
    X_eval_raw, _ = sem_test(N=args.n_eval, intervention=True, seed=seed + TEST_SEED_OFFSET)
    ...
    logger.info(f"leg 1: gamma = {gamma:.6g} at coverage {gamma_sel['record'].coverage:.4f}")
    ...
    if ceiling < target:
        ...
        logger.warning(
            f"DA+PI covers {ceiling:.4f} < target {target} at gamma "
    ...
    def budget_leg(name, attr, label):
        ...
        selected = min_knob_for_coverage(evaluate, lo, hi, target=aim, max_iter=args.max_iter, tol=args.tol)
    ...
    for name in ("PI&DA+PI", "PI&DA+PI+IV"):
        ...
        empty = float(record.status_counts[STATUS_CATEGORIES.index("infeasible")] / max(args.n_eval, 1))
        intersections[name] = {"coverage": record.coverage, "mean_width": record.interval_width, "empty_rate": empty}
        se = float(np.sqrt(max(parent * (1 - parent), 0.0) / max(args.n_eval, 1)))
        if record.coverage < parent - 2 * se:
            warnings.append(
                f"{name} covers {record.coverage:.4f} vs its parent "
    ...
    report = {
        ...
        "reference": {
            ...
            "DA+PI": {"coverage": da_pi_record.coverage, "mean_width": da_pi_record.interval_width},
        },
        "selected": {
            "gamma": leg_report(gamma_sel),
            "epsilon_iv": leg_report(
                iv_sel,
                floor=iv_floor,
                reference_width=da_pi_record.interval_width,
                ceiling=ceiling,
                target_reachable=ceiling >= target,
            ),
            "epsilon": leg_report(
                inv_sel,
                floor=inv_floor,
    ...
    report["reference"]["PI_vs_analytic_coverage"] = float(
        np.mean(_covered(pi.predict(X_eval, gamma=gamma), estimand_analytic))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--smoke", action="store_true", help="60k draws / 6k n_pi / 2k eval: minutes, for gates")
    ...
    if parsed.smoke:
        ...
        parsed.out = OUT_DEFAULT.replace(".json", "_smoke.json") if parsed.out == OUT_DEFAULT else parsed.out
    main(parsed)
