...
def direct_solves():
    ...
    y = X @ np.random.randn(6, 1) + .3 * np.random.randn(200, 1)
    GX = X + .2 * np.random.randn(*X.shape)
    Q = np.random.randn(60, 6)

    cases = {
        'PI': lambda nj: sm.PartialR2(gamma=.5, epsilon=.3, n_jobs=nj).fit(X, y),
        'PI_INV': lambda nj: sm.InvarianceConstrainedPartialR2(
            gamma=.5, epsilon=.3, n_jobs=nj).fit(X, y, GX=GX),
        'DA+PI_IV': lambda nj: sm.InstrumentalVariablePartialR2(
            gamma=.5, epsilon=.3, epsilon_iv=.2, n_jobs=nj).fit(GX, y, Z=GX),
        'PI&DA+PI': lambda nj: sm.IntersectedPartialR2(
            gamma=.5, epsilon=.3, n_jobs=nj).fit(X, y, GX=GX, G=GX),
    }
    out = {}
...
def optical_sweep():
    ...
    orchestrator = OpticalOrchestrator(
        seed=69, n_samples=200, n_experiments=2, sweep_samples=3,
        methods=['PI', 'PI_INV', 'DA+PI', 'PI&DA+PI'], hyperparameters={},
        n_jobs=1, augmentation='rotation > hflip > vflip > gaussian-noise',
    ...
    return {
        name: {
            ...
            **{metric: _digest(values) for metric, values in record.items()
...
if __name__ == '__main__':
    print(json.dumps({'direct': direct_solves(), 'optical': optical_sweep()},
