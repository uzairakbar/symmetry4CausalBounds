...
TRAIN_KW = dict(lr=0.01, batch=256, epochs=1, optimizer='adam', loss='mse',
                betas=(0.7, 0.9), onecycle=True, weight_decay=0.0,
                label_smoothing=0.0)
...
def dump(stem):
    ...
    base = dict(gamma=GAMMA, gamma0=1.0, n_components=N_COMPONENTS, latent='fa',
                link='probit', calibrate_sigma=True, n_anchors=N_ANCHORS, n_dir=128,
                n_radii=6, solver='cvxpy', jax_grad=True, pad=False, n_jobs=1,
    ...
    models = {
        ...
        'PI_INV': InvCopSens(outcome_model=da_erm, epsilon=EPSILON,
                             **base).fit(GX_pi, y_pi, GX=X_pi),
        'DA+PI_IV': IVCopSens(outcome_model=da_erm, epsilon=EPSILON, delta='from_eps',
                              rho=None, gamma_z_star=0.0,
                              **base).fit(GX_pi, y_pi, Z=G_pi),
    }
    ...
    for name, model in models.items():
        ...
        intermediates[name] = {
            ...
            'floor': (None if model._budget() is None
                      else float(model.constraint_floor())),
        }

    np.savez_compressed(f'{stem}.npz', X_pi=X_pi, GX_pi=GX_pi, y_pi=y_pi, G_pi=G_pi,
                        Q=Q, **{f'b_{k}': v for k, v in bounds.items()})
    with open(f'{stem}.json', 'w') as fh:
        json.dump({'attainable': list(sem.attainable),
                   'intermediates': intermediates}, fh, indent=1)
...
def check(stem):
    ...
    from src.methods.regression import GradientDescentERM       # noqa: PLC0415
    from src.methods.copsens import (CopSensPI, RecentredInvCopSens,  # noqa: PLC0415
    ...
    X_pi, GX_pi, y_pi, G_pi, Q = (blob['X_pi'], blob['GX_pi'], blob['y_pi'],
                                  blob['G_pi'], blob['Q'])

    # clipy=False: SOURCE never clips CopSens bounds. The clipped run is compared
    # separately below, so the delta stays a KNOWN one.
    base = dict(gamma=GAMMA, n_components=N_COMPONENTS, link='probit', calibrate=True,
                n_anchors=N_ANCHORS, jax_grad=True, pad=False, n_jobs=1,
                mu_clip=attainable, clipy=False)

    models = {
        'PI': CopSensPI(outcome_model=erm, **base).fit(X_pi, y_pi),
        'DA+PI': CopSensPI(outcome_model=da_erm, **base).fit(GX_pi, y_pi),
        'PI_INV': RecentredInvCopSens(outcome_model=da_erm, epsilon=EPSILON,
                                      **base).fit(X_pi, y_pi, GX=GX_pi),
        'DA+PI_IV': IVConstrainedCopSens(outcome_model=da_erm, epsilon_iv=EPSILON,
                                         **base).fit(GX_pi, y_pi, Z=G_pi),
    }
    ...
    for name, model in models.items():
        ...
        print(f'[{"PASS" if ok else "FAIL"}] {name:9s} max|dbound| {delta:.3g} '
        ...
        for label, mine, theirs in (
            ...
            ('Sigma eigs', float(np.abs(eigs).max()),
             float(np.abs(want_mid['sigma_eigs']).max())),
            ...
            print(f'  [{flag}] {name:9s} {label:11s} {float(mine):.8g} '
                  f'vs {float(theirs):.8g}')
            if rel >= 1e-6:
    ...
    print(f'[{"PASS" if ok else "FAIL"}] clipy delta is exactly a clip to '
          f'[{clipped.y_min:g}, {clipped.y_max:g}]')
    if not ok:
        failed.append('clipy')
