...
- **`im-ci: 95` on the six sweep recipes.** Every sweep metric now reads the 95%
  Imbens-Manski CI around each bound (a bootstrap of the fitted units, a unit being
  a row and, on the fold sweep, a base row with its m augmented copies) instead of
  the raw bound, and the raw record sits beside it in
